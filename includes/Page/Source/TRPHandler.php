...
class TRPHandler {
	...
	public static function trp_get_template_meta_query( $order = null ) {
		...
		} elseif ( null === $order && isset( $_COOKIE['pll_language'] ) && 'en_US' !== sanitize_text_field( $_COOKIE['pll_language'] ) && is_admin() ) {
		...
		} elseif ( null !== $order && 'en_US' !== sanitize_text_field( $current_order_language ) && is_admin() ) {
			if ( 'he' == sanitize_text_field( $current_order_language ) ) {
				update_option( 'yaymail_direction', 'rtl' );
			} else {
				update_option( 'yaymail_direction', 'ltr' );
			}
